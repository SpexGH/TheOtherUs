...
namespace TheOtherRoles.Patches {
    ...
    //        if (Bomber.hasBomb != null) setPlayerOutline(Bomber.currentBombTarget, Bomber.color);
        ...
        public static void phantomCheck(bool isMeeting=false)
        {
            if (PhantomAbility.phantomAbility == null) return;
            if (PhantomAbility.phantomAbility.Data.IsDead && isMeeting)
            {
                MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(CachedPlayer.LocalPlayer.PlayerControl.NetId, (byte)CustomRPC.TurnToPhantom, Hazel.SendOption.Reliable, -1);
        ...
        public static void losePhantomAbility()
        {
            if (PhantomAbility.phantomAbility == null) return;
            foreach (PlayerControl p in PlayerControl.AllPlayerControls)
            if (!Helpers.isNeutralForPhantom(p) && p == PhantomAbility.phantomAbility)
            {
    ...
    //        setPlayerOutline(Cultist.currentTarget, Palette.ImpostorRed);
        ...
        {
            if (Swooper.isInvisable && Swooper.swoopTimer <= 0 && Camouflager.camouflageTimer <= 0f && Swooper.swooper == CachedPlayer.LocalPlayer.PlayerControl)
            {
        ...
        public static void updatePlayerInfo() {
            foreach (PlayerControl p in CachedPlayer.AllPlayers) {         
                if ((Lawyer.lawyerKnowsRole && CachedPlayer.LocalPlayer.PlayerControl == Lawyer.lawyer && p == Lawyer.target) || p == CachedPlayer.LocalPlayer.PlayerControl || CachedPlayer.LocalPlayer.Data.IsDead || ((CachedPlayer.LocalPlayer.PlayerControl == Slueth.slueth && Slueth.reported.Any(x => x.PlayerId == p.PlayerId))) || MapOptions.impostorSeeRoles && Spy.spy == null && CachedPlayer.LocalPlayer.Data.Role.IsImpostor && !CachedPlayer.LocalPlayer.Data.IsDead && p == (p.Data.Role.IsImpostor && !p.Data.IsDead)) {
                    ...
                    if (p == CachedPlayer.LocalPlayer.PlayerControl || (MapOptions.impostorSeeRoles && Spy.spy == null && CachedPlayer.LocalPlayer.Data.Role.IsImpostor && !CachedPlayer.LocalPlayer.Data.IsDead && p == (p.Data.Role.IsImpostor  && !p.Data.IsDead))) {
	...
	static void arsonistUpdate() {
        ...
        static void amnisiacUpdate() {
            if (Amnisiac.amnisiac == null || CachedPlayer.LocalPlayer.PlayerControl != Amnisiac.amnisiac || Amnisiac.localArrows == null || !Amnisiac.showArrows) return;
        ...
        static void radarUpdate() {
            if (Radar.radar == null || CachedPlayer.LocalPlayer.PlayerControl != Radar.radar || Radar.localArrows == null || !Radar.showArrows) return;
            if (Radar.radar.Data.IsDead) {
                foreach (Arrow arrow in Radar.localArrows) UnityEngine.Object.Destroy(arrow.arrow);
    ...
    class BodyReportPatch
    {
        static void Postfix(PlayerControl __instance, [HarmonyArgument(0)]GameData.PlayerInfo target)
        {
            ...
            bool isSluethReport = Slueth.slueth != null && Slueth.slueth == CachedPlayer.LocalPlayer.PlayerControl && __instance.PlayerId == Slueth.slueth.PlayerId;
