...
public static class JesterEnterVent
{
    ...
    {
        if (Jester.jester == PlayerControl.LocalPlayer && Jester.canVent)
            return false;
        return true;
    }
}
...
public static class VentUsePatch
{
    public static bool Prefix(Vent __instance)
    {
        ...
        if (__instance.name.StartsWith("JackInTheBoxVent_"))
        {
            ...
            MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.UseUncheckedVent, SendOption.Reliable);
...
class KillButtonDoClickPatch
{
    public static bool Prefix(KillButton __instance)
    {
        ...
        if (__instance.isActiveAndEnabled && __instance.currentTarget && !__instance.isCoolingDown && !PlayerControl.LocalPlayer.Data.IsDead && PlayerControl.LocalPlayer.CanMove)
        {
            ...
            MurderAttemptResult res = checkMurderAttemptAndKill(PlayerControl.LocalPlayer, __instance.currentTarget);
            // Handle blank kill
...
public static class ConsoleCanUsePatch
{
    ...
    {
        canUse = couldUse = false;
        if (Swapper.swapper != null && Swapper.swapper == PlayerControl.LocalPlayer && !Swapper.canFixSabotages)
                    ...
                    if (plainShipRoom != null && plainShipRoom.roomArea)
                    {
                        ...
                        for (int j = 0; j < num; j++)
                        {
                                ...
                                if (bodyComponent)
                                {
                                    ...
                                    if (playerInfo != null)
                                    {
                                        ...
                                        if (Hacker.onlyColorType)
                                            color = isD(playerInfo.PlayerId) ? Palette.PlayerColors[7] : Palette.PlayerColors[6];
                            ...
                            else if (!collider2D.isTrigger)
                            {
                                ...
                                if (component && component.Data != null && !component.Data.Disconnected && !component.Data.IsDead && (__instance.showLivePlayerPosition || !component.AmOwner) && hashSet.Add(component.PlayerId))
                                {
                                    num2++;
                                    if (component?.cosmetics?.currentBodySprite?.BodySprite?.material != null)
                                    {
                                        ...
                                        if (Hacker.onlyColorType)
                                        {
                                            color = isLighterColor(component) ? Palette.PlayerColors[7] : Palette.PlayerColors[6];
                                        }
                                        roomColors.Add(color);
                                    }
                                }
...
class SurveillanceMinigamePatch
{
    ...
    private static Sprite overlaySprite = loadSpriteFromResources("TheOtherRoles.Resources.NightVisionOverlay.png", 350f);
    ...
    private static void nightVisionUpdate(SurveillanceMinigame SkeldCamsMinigame = null, PlanetSurveillanceMinigame SwitchCamsMinigame = null, FungleSurveillanceMinigame FungleCamMinigame = null)
    {
        ...
        if (nightVisionOverlays == null)
        {
            ...
            foreach (var renderer in viewPorts)
            {
                ...
                {
                    overlayObject = UnityEngine.Object.Instantiate(closeButton, renderer.transform);
                ...
                {
                    overlayObject = UnityEngine.Object.Instantiate(closeButton, viewablesTransform);
                ...
                UnityEngine.Object.Destroy(overlayObject.GetComponent<CircleCollider2D>());
        ...
        bool ignoreNightVision = (CustomOptionHolder.camsNoNightVisionIfImpVision.getBool() && hasImpVision(GameData.Instance.GetPlayerById(PlayerControl.LocalPlayer.PlayerId))) || PlayerControl.LocalPlayer.Data.IsDead;
