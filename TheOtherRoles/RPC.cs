...
public static class RPCProcedure
{
    ...
    public static void shifterShift(byte targetId)
    {
        ...
        if ((player.Data.Role.IsImpostor || isNeutral(player)) && !oldShifter.Data.IsDead)
        {
            oldShifter.Exiled();
    ...
    {
        PlayerControl target = playerById(playerId);
    ...
    public static void aftermathDead(byte playerId, byte killerId)
    {
        ...
        else if (Terrorist.terrorist == killer)
        {
            if (checkMuderAttempt(Terrorist.terrorist, Terrorist.terrorist) != MurderAttemptResult.BlankKill)
            {
                ...
                var writer = AmongUsClient.Instance.StartRpcImmediately(killer.NetId, (byte)CustomRPC.PlaceBomb, SendOption.Reliable);
    ...
    else if (Warlock.warlock == killer)
    {
        ...
        else if (Yoyo.yoyo == killer)
        {
            ...
            if (Yoyo.markedLocation == null)
            {
                ...
                var writer = AmongUsClient.Instance.StartRpcImmediately(killer.NetId, (byte)CustomRPC.YoyoMarkLocation, SendOption.Reliable);
                writer.WriteBytesAndSize(buff);
                AmongUsClient.Instance.FinishRpcImmediately(writer);
            ...
            else
            {
                ...
                var writer = AmongUsClient.Instance.StartRpcImmediately(killer.NetId, (byte)CustomRPC.YoyoBlink, SendOption.Reliable);
        ...
        else if (Swooper.swooper == killer)
        {
            ...
            swooperSwoopButton.Timer = swooperSwoopButton.MaxTimer + Jackal.duration;
        }/*
    ...
    public static void setInvisible(byte playerId, byte flag)
    {
        ...
        if (flag == byte.MaxValue)
        {
            ...
            if (Camouflager.camouflageTimer <= 0 && !MushroomSabotageActive() && !isCamoComms()) target.setDefaultLook();
    ...
    */
    ...
    public static void setInvisibleGen(byte playerId, byte flag)
    {
        ...
        if (flag == byte.MaxValue)
        {
            ...
            target.cosmetics.colorBlindText.gameObject.SetActive(DataManager.Settings.Accessibility.ColorBlindMode);
            target.cosmetics.colorBlindText.color = target.cosmetics.colorBlindText.color.SetAlpha(1f);
            if (Camouflager.camouflageTimer <= 0 && !MushroomSabotageActive()) target.setDefaultLook(); // testing
            return;
        }
    ...
    {
        PlayerControl sender = playerById(senderId);
        ...
        switch (infoType)
        {
            ...
            case GhostInfoTypes.DeathReasonAndKiller:
                overrideDeathReasonAndKiller(playerById(reader.ReadByte()), (DeadPlayer.CustomDeathReason)reader.ReadByte(), playerById(reader.ReadByte()));
                break;
        }
    }
