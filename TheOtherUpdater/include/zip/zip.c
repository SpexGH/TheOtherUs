...
static constexpr char* const zip_errlist[30] = {
    NULL,
    "not initialized\0",
...
{
    const char* p;
    const char* base = name += FILESYSTEM_PREFIX_LEN(name);
    int all_slashes = 1;
...
static char* zip_strrpl(const char* str, size_t n, char oldchar, char newchar)
{
    char c;
    size_t i;
    auto rpl = calloc(1 + n, sizeof(char));
    char* begin = rpl;
    if (!rpl)
    {
        return NULL;
                               ...
                               void* arg)
{
    ...
    memset(&info, 0, sizeof(mz_zip_archive_file_stat));
  ...
  strcpy(path, dir);
    ...
    strncpy(&path[dirlen], info.m_filename, MAX_PATH - dirlen);
        ...
        if ((((info.m_version_made_by >> 8) == 3) ||
            ...
            (0x20 << 24))
        {
            // and has sym link attribute (0x80 is file, 0x40
            // is directory)
            #if defined(_WIN32) || defined(__WIN32__) || defined(_MSC_VER) ||              \
    defined(__MINGW32__)
            #else
...
{
    ssize_t i = 0;
    auto local_header_ofs_array = calloc(n, sizeof(mz_uint64));
    ...
    auto length = calloc(n, sizeof(size_t));
    if (!length)
    {
...
{
    ssize_t n = 0;
    constexpr size_t page_size = 1 << 12; // 4K
    auto move_buf = calloc(1, page_size);
    if (!move_buf)
    {
        return ZIP_EOOMEM;
    }
    ...
    while (i < entry_num)
    {
        ...
        for (j = end; j < entry_num; j++)
        {
            MZ_ZIP_ARRAY_ELEMENT(&pState->m_central_dir_offsets, mz_uint32,
                                 begin + k) =
                MZ_ZIP_ARRAY_ELEMENT(&pState->m_central_dir_offsets,
                                     mz_uint32, j);
            k++;
        }
        d_num += end - begin;
                                       ...
                                       int entry_num)
{
    ...
    auto deleted_entry_flag_array =
        calloc(entry_num, sizeof(mz_bool));
    ...
    zip->archive.m_total_files =
        (mz_uint32)entry_num - deleted_entry_num;
...
struct zip_t* zip_open(const char* zipname, int level, char mode)
{
    ...
    zip = (struct zip_t*)calloc(1, sizeof(struct zip_t));
    if (!zip)
        goto cleanup;
...
int zip_is64(struct zip_t* zip)
{
    ...
    return zip->archive.m_pState->m_zip64;
}
                           ...
                           int case_sensitive)
{
  ...
  zip->entry.external_attr = (mz_uint32)(0100644) << 16;
    ...
    if (!mz_zip_writer_create_local_dir_header(
        ...
        pzip, zip->entry.header, entrylen, extra_size, 0, 0, 0,
        zip->entry.method,
...
int zip_entry_close(struct zip_t* zip)
{
    ...
    if (!mz_zip_writer_add_to_central_dir(
        pzip, zip->entry.name, entrylen, pExtra_data, extra_size,
...
int zip_entry_isdir(struct zip_t* zip)
{
    ...
    return mz_zip_reader_is_file_a_directory(&zip->archive,
...
int zip_entry_write(struct zip_t* zip, const void* buf, size_t bufsize)
{
    ...
    if (buf && bufsize > 0)
    {
        ...
        zip->entry.uncomp_crc32 = (mz_uint32)mz_crc32(
            zip->entry.uncomp_crc32, buf, bufsize);
...
int zip_entry_fwrite(struct zip_t* zip, const char* filename)
{
    ...
    memset(&file_stat, 0, sizeof(struct MZ_FILE_STAT_STRUCT));
...
struct zip_t* zip_stream_open(const char* stream, size_t size, int level,
                              char mode)
{
    auto zip = calloc(1, sizeof(struct zip_t));
    if (!zip)
    {
        return NULL;
    }
...
int zip_create(const char* zipname, const char* filenames[], size_t len)
{
    ...
    if (!memset(&file_stat, 0, sizeof(struct MZ_FILE_STAT_STRUCT)))
    {
        return ZIP_EMEMSET;
    }
